...
Copy multimedia resources to new directory. Resources are renamed according to objID.
    cpMume.py --input mume.xsl # copies resources to directory of input file
    
    This version renames images according to __mulId__.
    
    Expects the following excel columns: MMMulId, MMPfadangabe, MMDateiname, MMErweiterung
...
#
    ...
    c=0
    for r in td.table:
        #if r[mulId] == r[Stdbild]: work on every record
            fpath=os.path.join(r[c1],r[c2] ) + '.' + r[c3]
            out=os.path.join(outpath, str(r[mulId])+ '.' + r[c3])
            cpFile(fpath, out)
            #verbose ('-->'+ out)
            c+=1
